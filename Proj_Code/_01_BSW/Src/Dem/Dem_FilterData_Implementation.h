...
#if (DEM_FEATURE_NEED_REPORT_CHRONOLOGICALLY == STD_ON)
  Dem_Internal_AssertReturnValue(FilterId < Dem_Cfg_GetSizeOfReportedEventsOfFilter(), 
  ...
  )
{
  ...
  while (lPermanentIndex != 0)
  {
    ...
    { /* Visible Event identified, return it and stop processing */
      ...
      {
        if  ((Dem_Event_TestMilSupport(lPermanentEventId) == TRUE) && (Dem_DTC_TestSuppressionOnDtcRetrieval(lPermanentEventId) == FALSE))    /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
        {
          break;
        }
        else
    ...
    while (lPermanentIndex != 0)
    {
      ...
      if (DEM_ESM_TEST_PERMANENT_VISIBLE(Dem_Mem_PermanentGetStatus(lPermanentIndex)) == TRUE)
      {
        if ( (Dem_Mem_PermanentGetEvent(lPermanentIndex) != DEM_EVENT_INVALID)                                                   /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
          ...
          && (Dem_DTC_TestSuppressionOnDtcRetrieval(Dem_Mem_PermanentGetEvent(lPermanentIndex)) == FALSE))
        {
          ++lNumberOfEvents;
        }
      }
...
# if (DEM_CFG_REPORT_SUPPRESSED_DTCS_IN_19_0A == STD_ON)
    if ((Dem_FilterData_TestUseStatusMask(FilterId) == FALSE)                                                                    /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
      && (Dem_FilterData_GetMemoryId(FilterId) == DEM_CFG_MEMORYID_PRIMARY)
      && (Dem_DTC_TestDtcSuppression(lNextEvent) == TRUE)
      )      
    {
