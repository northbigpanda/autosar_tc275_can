  ...
  )
{
...
#if (DEM_CFG_SUPPORT_IUMPR == STD_ON)
  {
    ...
    if (lRatioIndex != DEM_CFG_RATIOINDEX_INVALID)
    {
       ...
       * - previous value of general conditions fulfilled since last 500MI increment is maintained
       */
      uint8 lRatioStatus;
      lRatioStatus = Dem_Mem_GetIumprStatus(lRatioIndex);
      lRatioStatus = ((lRatioStatus) & (DEM_IUMPRIF_IUMPR_500MI_GENDEN_CONDITIONS_REACHED));
...
#if (DEM_CFG_SUPPORT_WWHOBD == STD_ON)
  if ((EventContext->EventEffects & DEM_CFG_TRIGGER_PASSED) != FALSE)
  {
...
# if (DEM_CFG_AGING_RETAIN_MEMORY_ENTRY == STD_ON)
    ...
    Dem_DTC_SetGroupProcessed(lCurrentlyProcessingGroupId, 
                              (Dem_DTC_OperationType)Dem_DTC_QualificationProcessing);
  ...
  )
{
  ...
  if (Dem_Cfg_EventLatchTestFailed(EventId) == TRUE)
  {
    ...
    lReturnValue =
      (boolean)(((lStatusMask)& Dem_Event_GetUDSStatus(EventId)) != 0);
  }
  else
  {
    lReturnValue = FALSE;
