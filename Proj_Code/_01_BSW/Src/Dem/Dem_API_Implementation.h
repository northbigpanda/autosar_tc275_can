...
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetEnableCondition(
  uint8  EnableConditionID,                                                                                                        /* PRQA S 1330 */ /* MD_DEM_16.4_rte */
...
# if (DEM_DEV_ERROR_DETECT == STD_ON)
  if (Dem_InitState_TestMasterInState(DEM_INITSTATE_INITIALIZED) == FALSE)
  {
    ...
    if ((EnableConditionID == DEM_ENABLECONDITION_CONTROLDTCSETTING)                                                                /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
...
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetStorageCondition(
  uint8  StorageConditionID,                                                                                                       /* PRQA S 1330 */ /* MD_DEM_16.4_rte */
  boolean  ConditionFulfilled
  )
{
  Std_ReturnType lReturnValue;
...
# if (DEM_CFG_SUPPORT_500MI_DENOMINATOR == STD_ON)        
          if (Dem_IumprIF_Test500MileDenominatorConfigured() == TRUE)
          {
            /* Update the general denominator reached since last increment for 500MI denominator */
...
# endif
       ...
       )
    {
           ...
           Dem_Cfg_ObdRatioIdIterNext(&lRatioIter))                                                                              /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        ...
        lRatioDenominator = Dem_Cfg_ObdIumprDenGroup(lRatioIndex);                                    
...
/* >>>> -------------------------------- Enter Critical Section: DiagMonitor */
          ...
          if (ConditionStatus == DEM_IUMPR_DEN_STATUS_REACHED)
          {
            ...
            if ((Dem_IumprIF_TestGeneralDenominatorIncrementConditionFulfilled(lRatioIndex) == TRUE)
                && (DEM_ESM_TEST_IUMPR_DEN_INCREMENTED(lRatioStatus) == FALSE))
            {
              /* conditions for increment seem to be fulfilled; set pending flag and increment on task level */
              Dem_Mem_IumprSetUpdatePending(lRatioIndex, TRUE);                                      
...
/* <<<< -------------------------------- Leave Critical Section: DiagMonitor */
    ...
    else if ((ConditionId != DEM_IUMPR_GENERAL_DENOMINATOR) 
      ...
      *NumberOfFilteredRatios = 0;                                                                                               /* SBSW_DEM_POINTER_WRITE_API */
        ...
        ++lEventId)
      {
        ...
        if ((lRatioIndex != DEM_CFG_RATIOINDEX_INVALID)                                                                          /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
          && (Dem_Event_TestSuppressionOnEventRetrieval(lEventId) == FALSE)
          )
        {
