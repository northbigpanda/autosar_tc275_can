  ...
  )
{
  ...
  {
    lSelectedIndex = Dem_MemObdFreezeFrame_SelectFFDisplaceIndex(lMemoryRepresentativeEventId, DisplacePendingFreezeFrame);
  }
...
# if (DEM_CFG_SUPPORT_OBDII == STD_ON)
  ...
  while (lFreezeFrameIndex != 0U)
  {
    ...
    if (lEventId != DEM_EVENT_INVALID)
    {
      if ( (Dem_MemObdFreezeFrame_IsFreezeFrameStorageValid(lEventId) == FALSE)                                                  /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
  ...
  )
{
  ...
  if ((Dem_UDSStatus_Test_CDTC(Dem_DTC_RetrieveStatus(lEventId)) == FALSE)                                                       /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
