...
\ID SBSW_DEM_ARRAY_WRITE_PROCESSEDCOMBINEDDTCGROUPS
  \DESCRIPTION    In a function, data is written to array Dem_Cfg_ProcessedCombinedDTCGroups[][] using function arguments as indices.
                  The ComStackLib guarantees that the size of the outer array Dem_Cfg_ProcessedCombinedDTCGroups[] is correctly 
                  returned by the GetSize access macro. 
  \COUNTERMEASURE \R [CM_DEM_R_PROCESSEDCOMBINEDDTCGROUPS_INDEX_CHECK]
                  \M [CM_DEM_M_SIZEOF_PROCESSEDCOMBINEDDTCGROUPS]
...
\ID SBSW_DEM_POINTER_FORWARD_PROCESSED_COMBINEDDTC_GROUPS
  \DESCRIPTION    A pointer to the inner array of Dem_Cfg_ProcessedCombinedDTCGroups is passed with its corrresponding size to a 
                  function expecting a valid pointer and size.
                  The structure array is generated from ComStackLib and the pointer is validated using ComStackLib function 
                  before usage. Hence the pointer is always valid.
  \COUNTERMEASURE \M [CM_DEM_M_SIZEOF_PROCESSEDCOMBINEDDTCGROUPS]
...
\CM CM_DEM_M_SIZEOF_PROCESSEDCOMBINEDDTCGROUPS
  Verify Dem_Cfg_ProcessedCombinedDTCGroups[][] is defined for each index in range [0..DEM_CFG_PROCESSEDCOMBINEDDTCGROUP_SIZE[ .
...
\CM CM_DEM_R_PROCESSEDCOMBINEDDTCGROUPS_INDEX_CHECK
  Verify that all array indices are in range before writing to Dem_Cfg_ProcessedCombinedDTCGroups[][] .
