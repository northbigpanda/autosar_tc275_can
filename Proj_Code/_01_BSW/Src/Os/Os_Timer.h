...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  Timer->Dyn->OverflowCount = 0;                                                                                        /* SBSW_OS_TIMER_TIMERGETDYN_001 */
  Timer->Dyn->CompareReached = FALSE;                                                                                   /* SBSW_OS_TIMER_TIMERGETDYN_001 */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  Timer->Dyn->HwTimeAtLogicalZero = Os_TimerFrtTickType2TickType(timeZero);                                             /* SBSW_OS_TIMER_TIMERGETDYN_001 */
  ...
  Timer->Dyn->LastCounterValue = Os_TimerFrtTickType2TickType(timeZero);          /* store last counter value.      */  /* SBSW_OS_TIMER_TIMERGETDYN_001 */
...
TickType, OS_CODE, OS_ALWAYS_INLINE, Os_TimerHrtTickType2FrtTickType,
(
  P2CONST(Os_TimerHrtConfigType, AUTOMATIC, OS_CONST) Timer,
  TickType Value
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  return (Timer->Dyn->HwTimeAtLogicalZero + Value);
}


/***********************************************************************************************************************
 *  Os_TimerHrtGetHardwareCompareValue()
 **********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
...
OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE, Os_TimerHrtGetHardwareCompareValue,
(
  P2CONST(Os_TimerHrtConfigType, AUTOMATIC, OS_CONST) Timer,
  Os_TickType ExpirationTime
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  Os_TickType result = ExpirationTime;

  /* #10 If the size of the physical counter value type is smaller then the OS TickType: */
  if(Os_TimerIsFrtShortCorrectionNeeded() != 0u)                                                                        /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
  {
    Os_TickType now;

    /* #20 Reset the timers overflow counter to zero. */
    Timer->Dyn->OverflowCount = 0u;                                                                                     /* SBSW_OS_TIMER_TIMERGETDYN_001 */

    /* #30 Tell the timer that the logical compare value is equal to the hardware compare value. */
    Timer->Dyn->CompareReached = TRUE;                                                                                  /* SBSW_OS_TIMER_TIMERGETDYN_001 */

    /* #40 Get the current timer value. */
    now = Os_TimerFrtGetCounterValue(Timer, FALSE);                                                                     /* SBSW_OS_FC_PRECONDITION */

    /* #50 If the expiration time is a future value: */
    if(Os_CounterIsFutureValue(&(Timer->Counter), ExpirationTime, now) != 0u)                                           /* SBSW_OS_FC_PRECONDITION */
    {
      /* #60 Update the timers overflow counter. */
      Os_TickType diff = ExpirationTime - now;
      Timer->Dyn->OverflowCount = (diff + Os_TimerTickType2FrtTickType(now)) >> Os_TimerFrtGetTimerBitWidth();          /* SBSW_OS_TIMER_TIMERGETDYN_001 */

      /* #70 If the timer needs to overflow to reach the expiration time: */
      if(Timer->Dyn->OverflowCount > 0u)
      {
        /* #80 Tell the timer that the logical compare value is NOT equal to the hardware compare value. */
        Timer->Dyn->CompareReached = FALSE;                                                                             /* SBSW_OS_TIMER_TIMERGETDYN_001 */

        /* #90 Set the hardware compare value to zero for overflow handling. */
        result = 0u;
      }
    }
  }
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  /* #15 Get the compare value that needs to be written to the hardware register. */
  correctedExpirationTime = Os_TimerHrtGetHardwareCompareValue(Timer, correctedExpirationTime);                         /* SBSW_OS_FC_PRECONDITION */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  if(Os_TimerIsFrtShortCorrectionNeeded() != 0u)                                                                        /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
  {
    ...
    /* #30 If the interrupt is an overflow signal: */
    if(Timer->Dyn->OverflowCount > 0u)
    {
      /* #40 Get the current logical value with overflow correction forced. */
      now = Os_TimerFrtGetCounterValue(Timer, TRUE);                                                                    /* SBSW_OS_FC_PRECONDITION */
    }
    /* #50 Otherwise: */
    else
    {
      /* #60 Get the current logical value. */
      now = Os_TimerFrtGetCounterValue(Timer, FALSE);                                                                   /* SBSW_OS_FC_PRECONDITION */
    }

    /* #70 Update the OS internal logical counter values. */
    Os_TimerHrtUpdate(Timer, now);                                                                                      /* SBSW_OS_FC_PRECONDITION */

    /* #80 If all overflow signals are handled: */
    if(Timer->Dyn->OverflowCount == 0u)
    {
      /* #90 If the requested logical compare value is reached: */
      if(Timer->Dyn->CompareReached == TRUE)
      {
        /* #100 Set the result to TRUE. */
        result = !0u;                                                                                                   /* PRQA S 4404, 4558 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule10.1_4558 */

        /* #110 Update the compare value to recognize the next overflow. */
        Timer->Dyn->OverflowCount = Timer->Counter.Characteristics.MaxCountingValue;                                    /* SBSW_OS_TIMER_TIMERGETDYN_001 */
        Os_TimerFrtSetCompareValue(Timer->HwConfig, 0u);                                                                /* SBSW_OS_TIMER_TIMERFRTSETCOMPAREVALUE_001 */
      }
      /* #120 Otherwise: */
      else
      {
        /* #130 Update the hardware compare value in accordance to the logical compare value. */
        Os_TimerHrtSetCompareValue(Timer, Timer->Dyn->Compare);                                                         /* SBSW_OS_FC_PRECONDITION */
      }
    }
  }
  /* #140 Otherwise: */
  ...
  {
    /* #150 The hardware compare value is equal to the OS internal compare value, so the result is TRUE. */
    result = !0u;                                                                                                       /* PRQA S 4404, 4558 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule10.1_4558 */
  }
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  if(Os_TimerIsFrtShortCorrectionNeeded() != 0u)                                                                        /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
  {
    /* #30 Update the upper bits according to the current logical counter value. */
    now += (Timer->Dyn->Value & Os_TimerFrtGetShortValueMask());

    /* #40 If an overflow has been detected: */
    if((now < Timer->Dyn->Value) || (OverFlowOccurred == TRUE))
    {
      /* #50 Update the upper bits due to the overflow. */
      now += OS_FRT_TICKTYPE_MAX + 1u;                                                                                  /* PRQA S 2910 */ /* MD_Os_Rule12.4_2910 */ /* COMP_WARN_OS_OVERFLOW */

      /* #60 If the timers overflow counter is larger then zero: */
      if(Timer->Dyn->OverflowCount > 0u)
      {
        /* #70 Decrement the timers overflow counter. */
        Timer->Dyn->OverflowCount--;                                                                                    /* SBSW_OS_TIMER_TIMERGETDYN_001 */
      }
    }
  }

  return (now  & (Timer->Dyn->ValueMask));
}
...
(
  P2CONST(Os_TimerHrtConfigType, AUTOMATIC, OS_CONST) Timer,
  Os_TickType CurrentTimeStamp
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  Os_TickType timePassed;

  /* #10 Calculate how much time has passed, since we have read from the hardware the last time. */
  timePassed = (CurrentTimeStamp - Timer->Dyn->LastCounterValue);
  Timer->Dyn->LastCounterValue = CurrentTimeStamp;                                                                      /* SBSW_OS_TIMER_TIMERGETDYN_001 */

  /* #20 Add this time to the current counter value. */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  /* #10 Get the current counter value. */
  now = Os_TimerFrtGetCounterValue(Timer, FALSE);                                                                       /* SBSW_OS_FC_PRECONDITION */

  /* #20 Update the internal counter representation. */
  Os_TimerHrtUpdate(Timer, now);                                                                                        /* SBSW_OS_FC_PRECONDITION */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  if(Os_TimerIsFrtLongCorrectionNeeded() != 0u)                                                                         /* COV_OS_HALPLATFORMFRT64BITAVAILABILITY */
  {
    ...
    compareValue = (now & Os_TimerFrtGetLongValueMask()) + Os_TimerTickType2FrtTickType(CompareValue);
...
/***********************************************************************************************************************
 *  Os_TimerFrtGetShortValueMask()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE,                                    /* PRQA S 3219 */ /* MD_Os_Rule2.1_3219 */ /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
Os_TimerFrtGetShortValueMask,(void))                                                                                    /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
{
  return (Os_TickType)(OS_OS_TICKTYPE_MAX - OS_FRT_TICKTYPE_MAX);
}


/***********************************************************************************************************************
 *  Os_TimerFrtGetTimerBitWidth()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE,                                    /* PRQA S 3219 */ /* MD_Os_Rule2.1_3219 */
Os_TimerFrtGetTimerBitWidth,(void))                                                                                     /* COV_OS_HALPLATFORMFRT16BITAVAILABILITY */
{
  return OS_HAL_TIMERFRTBITWITDH;
}
