...
typedef struct
{
  ...
  /*! The number of overflows until a specific compare value is reached.
   *  This value has to be evaluated during timer interrupt handling. */
  Os_TickType OverflowCount;

  /*! This value indicates if the next logical time stamp (Compare value) has been reached.
   *  This value is set in accordance to the timers OverflowCount. */
  boolean CompareReached;
 ...
 **********************************************************************************************************************/
/*! \brief          Get the current counter value of an FRT and update the internal overflow counter if needed.
 *  \details        The function returns the OS internal logical counter value. If hardware counter overflow handling
 *                  is needed, it is done here.
 *
 *  \param[in/out]  Timer             The timer to query. Parameter must not be NULL.
 *  \param[in]      OverFlowOccurred  Indicates if the functions is called after an overflow:
 *                                    TRUE:  Function is called due to an timer overflow indicated by an interrupt.
 *                                    FALSE: Decision about an overflow needs to be done according to the counter value.
 *
 *  \return         The current logical counter value of the given timer module.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  Os_TimerHrtUpdate()
 **********************************************************************************************************************/
/*! \brief          Updates the internal values of the according HRT counter.
 *  \details        --no details--
 *
 *  \param[in,out]  Timer             The timer to update. Parameter must not be NULL.
 *  \param[in]      CurrentTimeStamp  The time stamp information.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE for different timers.
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE TickType, OS_CODE, OS_ALWAYS_INLINE,
Os_TimerHrtTickType2FrtTickType,
(
  P2CONST(Os_TimerHrtConfigType, AUTOMATIC, OS_CONST) Timer,
  TickType Value
 ...
 *  \details        Additionaly this function updates the internal timer values for overflow handling.
 *                  See \ref Os_TimerIsFrtShortCorrectionNeeded().
 *
 *  \param[in,out]  Timer           The timer to modify. Parameter must not be NULL.
 ...
 *                  OS TickType. In this case overflow handling is needed. The function evaluates and updates
 *                  the Timers internal overflow count and compare value information.
 *
 *  \param[in/out]  Timer
 *
 ...
 **********************************************************************************************************************/
/*! \brief          Returns the bit mask to access the upper bits of an OS internal timer value.
 *  \details        Returns a bit mask for the case that the OS Ticktype is larger than the hardware timer type.
 *
 *  \return         The bit mask to access the upper part of a logical timer value.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE,
Os_TimerFrtGetShortValueMask,(void));


/***********************************************************************************************************************
 *  Os_TimerFrtGetTimerBitWidth()
 **********************************************************************************************************************/
/*! \brief          Returns the width of the hardware timer register.
 *  \details        This function is needed if the size of the hardware timer type is smaller then the OS TickType.
 *
 *  \return         The bit width of the FRT registers.
 *                  Or 0 if the hardware timer type is larger or equal to the OS TickType.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE,
Os_TimerFrtGetTimerBitWidth,(void));
