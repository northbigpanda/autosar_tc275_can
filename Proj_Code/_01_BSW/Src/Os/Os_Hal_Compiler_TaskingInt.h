...
{ \
  ...
  tin = Os_Hal_Mfd15(); \
  addr = (Os_AddressOfConstType)Os_Hal_Mfa11(); \
  pcxi = Os_Hal_Mfcr(OS_HAL_PCXI_OFFSET); \
  savedStackRegionLow = Os_Hal_Mfcr(OS_HAL_COREMPU_DPR_LOW0);\
  savedStackRegionUpper = Os_Hal_Mfcr(OS_HAL_COREMPU_DPR_UPPER0);\
...
{ \
  ...
  Os_Hal_Mtcr(OS_HAL_PCXI_OFFSET, pcxi); \
  Os_Hal_Isync(); \
}
