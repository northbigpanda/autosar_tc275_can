  ...
  /* #20 For each core: */
  {
    ...
    for(coreId = (CoreIdType)0; coreId < OS_COREID_COUNT; coreId++)                                                     /* PRQA S 4332,  4527 */ /* MD_Os_Rule10.5_4332,  MD_Os_Rule10.1_4527 */
    {
      ...
      if(Os_CoreIsAsrCore(coreIt) != 0u)                                                                                /* SBSW_OS_CORE_COREISASRCORE_001 */
      {
        ...
        if (Os_BarrierIsEnabled() != 0u)
        {
          Os_ObjIdxType index;

          /* #80 For all configured barriers */
          for (index = 0; index < asrCore->BarrierCount; index++)
          {
...
))
{
  ...
  if (coreState == OS_CORESTATE_ACTIVATED_ASR)
  {
...
)
{
  ...
  for(interruptSourceIdx = (Os_ObjIdxType)0; interruptSourceIdx < SystemCfg->InterruptSourceCount; interruptSourceIdx++)
  {
    ...
    Os_IsrInterruptSourceInit(SystemCfg->InterruptSourceRefs[interruptSourceIdx]);                                      /* SBSW_OS_CORE_ISRINTERRUPTSOURCEINIT_001 */
  }
}
...
)
{
  ...
  {
    /* #20 Call low level initialization function. */
    Os_IsrInterruptSourceInit(Core->InterruptSourceRefs[interruptSourceIdx]);                                           /* SBSW_OS_CORE_ISRINTERRUPTSOURCEINIT_001 */
  }
}
