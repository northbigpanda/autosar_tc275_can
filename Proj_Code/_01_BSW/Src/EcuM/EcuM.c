...
#   if((ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON) && (ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF))
...
\ID COV_ECUM_PARTITIONID
  \ACCEPT  TX
  \REASON  The EcuM_ErrorHook is called in case that an application ID is returned which is out of range of EcuM_GetSizeOfPartitionData.
