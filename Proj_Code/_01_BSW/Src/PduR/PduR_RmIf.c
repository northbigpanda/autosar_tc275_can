 ...
 *  \synchronous TRUE
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(PduR_ActualLengthOfSingleBufferRamType, PDUR_CODE) PduR_RmIf_GetActualLengthOfSingleBufferRamSafe(PduR_SingleBufferRomIdxOfRmBufferedIfPropertiesRomType singleBufferRomIdx);
#endif
...
#   endif
            {
              ...
              if(PduR_McQ_WriteTxConfirmation(PduR_GetMcQBufferRomIdxOfSrcApplicationRom(PduR_GetSrcApplicationRomStartIdxOfDestApplicationManagerRom(destApplicationIdx) + SrcApplicationIdx), rmSrcRomIdx) == E_NOT_OK)
              {
...
FUNC(Std_ReturnType, PDUR_CODE) PduR_RmIf_SingleBuffer_Get(PduR_RmGDestRomIterType rmGDestRomIdx, P2VAR(PduR_RmDestRomIterType, AUTOMATIC, PDUR_APPL_DATA) rmDestRomIdx, P2VAR(PduInfoType, AUTOMATIC, PDUR_APPL_DATA) info)
{
  ...
  info->SduLength = PduR_RmIf_GetActualLengthOfSingleBufferRamSafe(singleBufferRomIdx); /* SBSW_PDUR_VALID_PTR_ENSURED_BY_CALLING_FUNCTION */
...
LOCAL_INLINE FUNC(PduR_ActualLengthOfSingleBufferRamType, PDUR_CODE) PduR_RmIf_GetActualLengthOfSingleBufferRamSafe(PduR_SingleBufferRomIdxOfRmBufferedIfPropertiesRomType singleBufferRomIdx)
{
  PduR_ActualLengthOfSingleBufferRamType actualLengthOfSingleBuffer = PduR_GetActualLengthOfSingleBufferRam(singleBufferRomIdx);

# if (PDUR_MIXED_ASIL_ERROR_CHECK == STD_ON) /* COV_PDUR_VAR_MIXED_ASIL_ERROR_CHECKS */
  PduR_SingleBufferArrayRamLengthOfSingleBufferRomType singleBufferArrayLength = PduR_GetSingleBufferArrayRamLengthOfSingleBufferRom(singleBufferRomIdx);
...
#  endif
          ...
          {
            if(PduR_McQ_WriteData(PduR_GetMcQBufferRomIdxOfSrcApplicationRom(PduR_GetSrcApplicationRomStartIdxOfDestApplicationManagerRom(destApplicationIdx) + SrcApplicationIdx), (PduR_RmDestRomEndIdxOfRmSrcRomType) rmDestIdx, info) == E_OK)     /* SBSW_PDUR_VALID_PTR_ENSURED_BY_CALLING_FUNCTION */
            {
              retVal &= E_OK;
            }
            else
...
LOCAL_INLINE FUNC(void, PDUR_CODE) PduR_RmIf_TxConfirmation(PduR_RmGDestRomIdxOfTxIf2UpType rmGDestRomIdx)
{
  ...
  {
    switch (PduR_GetIfSmStateOfRmBufferedIfPropertiesRam(PduR_GetRmBufferedIfPropertiesRomIdxOfRmGDestRom(rmGDestRomIdx)))
    {
