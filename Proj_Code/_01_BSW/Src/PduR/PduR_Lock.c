  ...
  if(!PduR_IsExclusiveAreaRomUsedOfLockRom(lockIdx))
  {
    SchM_Enter_PduR_PDUR_EXCLUSIVE_AREA_0();
    {
      ...
      if(core_id < (uint8) PDUR_MAX_NUMBER_OF_CORES)
      {
        ...
        {
          if(E_OK != Appl_GetSpinlock(PduR_GetAddrLockVariableOfSpinlockRam(spinlockIdx))) /* SBSW_PDUR_API_CALL_FORWARDING_ID_ONLY */ /* COV_PDUR_FATAL_ERROR */
          {
            PduR_Det_ReportError(PDUR_LOCK_LOCK, PDUR_E_FATAL);
          }
        }
        PduR_GetCounterOfSpinlockRam(spinlockIdx)[core_id]++;       /* SBSW_PDUR_CORE_ID_RTCHECK */
      }
      else
      {
        PduR_Det_ReportError(PDUR_LOCK_LOCK, PDUR_E_FATAL);
  ...
  {
    {
      ...
      if(core_id < (uint8) PDUR_MAX_NUMBER_OF_CORES)
      {
        ...
        {
          if(E_OK != Appl_ReleaseSpinlock(PduR_GetAddrLockVariableOfSpinlockRam(spinlockIdx)))     /* SBSW_PDUR_API_CALL_FORWARDING_ID_ONLY */ /* COV_PDUR_FATAL_ERROR */
          {
